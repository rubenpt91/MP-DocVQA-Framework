...
class VisualEmbeddings(nn.Module):
    def __init__(self, config, finetune=False):
        ...
        cfg = AutoConfig.from_pretrained(config.visual_module_config["model_weights"])
        ...
        if not finetune and not self.precomputed_visual_feats:
            self.freeze()
    ...
    def forward(self, images, page_idx_mask):
        if not hasattr(self, "device"):  # nasty hack to get correct device
            if self.precomputed_visual_feats:
                self.device = next(
                    self.visual_emb_matcher.layers[0].parameters()
                ).device
            else:
                self.device = self.image_model.device

        if self.precomputed_visual_feats:
            vis_embeddings = torch.Tensor(np.array(images)).to(self.device)
        else:
            inputs = self.feature_extractor(images=images, return_tensors="pt")
            vis_embeddings = self.image_model(inputs.pixel_values.to(self.device))
            vis_embeddings = (
        ...
        vis_attention_mask = torch.zeros(vis_embeddings.shape[:2]).to(self.device)
    ...
    def _prepare_encoder_decoder_kwargs_for_generation(
        ...
        for p_idx in range(max(model_kwargs["num_pages"])):
            ...
            page_idx_mask = [
                ...
                if num_pages[batch_idx] >= p_idx + 1
            ]
    ...
    def forward(
        ...
        if encoder_outputs is None:
            ...
            attention_mask = torch.zeros(
                [hidden_states.shape[0], self.page_tokens * max(num_pages)]
            ).to(
                document_embeddings.device
            )  # Pages, hidden size. Make use of all information of the document embedding
            for bs_idx in range(len(hidden_states)):
                attention_mask[
                    bs_idx,
                    : min(num_pages[bs_idx], self.max_doc_pages) * self.page_tokens,
                ] = 1
            ...
            hidden_states = encoder_outputs[
                0
            ]  # TODO - This should be replaced by document embeddings
            ...
            for bs_idx in range(len(hidden_states)):
                attention_mask[
                    bs_idx, : min(num_pages[bs_idx], max(num_pages)) * self.page_tokens
                ] = 1
...
class Proxy_HiVT5:
    def __init__(self, config):
        ...
        self.model = HiVT5.from_pretrained(
    ...
    def prepare_vqa_input_ids(self, batch):
        ...
        for batch_idx in range(bs):
            ...
            all_attention_masks[
                batch_idx, : num_pages[batch_idx], : tokens.attention_mask.shape[-1]
            ...
            for page_idx in range(num_pages[batch_idx]):
                if len(batch["boxes"][batch_idx][page_idx]) >= 1:
                    ...
                    context_boxes = context_boxes[
                        ...
                        - len(pretext_boxes)
                        - 1
                    ]  # Remove boxes out of model max length.
                ...
                all_boxes[
                    batch_idx,
                    page_idx,
                    self.page_tokens : self.page_tokens + len(pretext_boxes),
                ] = pretext_boxes
                ...
                all_boxes[
                    ...
                    page_idx,
                    self.page_tokens
                    + length_pretext : self.page_tokens
                    + length_pretext
                    + len(context_boxes),
                ] = (
                    context_boxes * 1000
                ...
                all_boxes[
                    batch_idx,
                    page_idx,
                    self.page_tokens + length_pretext + len(context_boxes),
                ] = torch.tensor(eos_box)
    ...
    def forward(
        ...
        if self.page_retrieval == "oracle":
            ...
            if (
                self.model.training or output_attentions
            ):  # TODO: Output attentions should be for inference (generate). But I can't output encoder attentions..
    ...
    def get_answer_from_model_output(
        ...
        output = self.model.generate(
            ...
            attention_mask=attention_mask,
            num_pages=num_pages,
            max_length=self.decoding_max_length,
    ...
    def forward_record_and_retrieve_attention_dict(self, record):
        ...
        with torch.no_grad():
            ...
            outputs, pred_answer, pred_answer_page = self.forward(
                batched_record, output_attentions=True, return_pred_answer=True
            )
        ...
        for page_idx in range(num_pages):
            ...
            if len(page_tokens) > self.max_text_tokens:
                page_tokens = page_tokens[: self.max_text_tokens - 1] + [
                    self.tokenizer.eos_token_id
                ]
                page_boxes = page_boxes[: self.max_text_tokens - 1] + [
        ...
        decoder_input_text = [
            ...
            for page_idx in range(num_pages)
            for token_idx in range(self.page_tokens)
        ]
